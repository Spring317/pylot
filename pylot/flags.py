...
flags.DEFINE_string(
    "csv_log_file_name", None, "csv file into which to log runtime stats"
)
flags.DEFINE_string(
    "profile_file_name",
    "pylot_profile.json",
...
flags.DEFINE_bool(
    "obstacle_detection", False, "True to enable obstacle detection operator"
)
flags.DEFINE_enum(
    "obstacle_detector_model",
    "faster-rcnn",
    ["faster-rcnn", "efficientdet", "yolo"],
    "Which detection backend to use when --obstacle_detection is set. "
    '"yolo" uses the YOLOv8 model at --yolo_detection_model_path; '
    '"efficientdet" uses the EfficientDet model; '
    '"faster-rcnn" (default) uses the TF Faster-RCNN saved model.',
)
flags.DEFINE_bool(
    "perfect_obstacle_detection", False, "True to enable perfect obstacle 2D detection"
)
flags.DEFINE_bool(
...
flags.DEFINE_bool(
    "obstacle_tracking", False, "True to enable obstacle tracking operator"
)
flags.DEFINE_bool(
    "perfect_obstacle_tracking", False, "True to enable perfect obstacle tracking"
)
flags.DEFINE_enum(
    "tracker_type",
    "sort",
...
flags.DEFINE_bool(
    "perfect_lane_detection", False, "True to enable perfect lane detection"
)
flags.DEFINE_enum(
    "lane_detection_type",
    "lanenet",
...
flags.DEFINE_bool(
    "traffic_light_detection", False, "True to enable traffic light detection operator"
)
flags.DEFINE_bool(
...
flags.DEFINE_bool("segmentation", False, "True to enable segmentation operator")
flags.DEFINE_bool("perfect_segmentation", False, "True to enable perfect segmentation")
flags.DEFINE_enum(
    "obstacle_location_finder_sensor",
    "lidar",
    ["lidar", "depth_camera", "depth_stereo"],
    "Sets which sensor to use to compute obstacle locations",
)
flags.DEFINE_bool("depth_estimation", False, "True to estimate depth using cameras")
flags.DEFINE_bool("perfect_depth_estimation", False, "True to use perfect depth")
flags.DEFINE_bool(
    "localization",
    False,
...
flags.DEFINE_enum(
    "prediction_type", "linear", ["linear", "r2p2"], "Type of prediction module to use"
)
...
######################################################################
flags.DEFINE_bool(
    "visualize_rgb_camera", False, "True to enable RGB camera sensor visualization"
)
flags.DEFINE_bool(
    "visualize_depth_camera", False, "True to enable depth camera sensor visualization"
)
flags.DEFINE_bool("visualize_lidar", False, "True to enable Lidar visualization")
flags.DEFINE_bool("visualize_imu", False, "True to enable simulator IMU visualization")
flags.DEFINE_bool(
    "visualize_pose", False, "True to enable simulator ego-vehicle pose visualization"
)
flags.DEFINE_bool(
...
flags.DEFINE_bool(
    "visualize_detected_lanes", False, "True to enable visualization of detected lanes"
)
flags.DEFINE_bool(
...
flags.DEFINE_bool(
    "visualize_segmentation", False, "True to enable visualization of segmentaation"
)
flags.DEFINE_bool(
    "visualize_waypoints", False, "True to enable visualization of planning waypoints"
)
flags.DEFINE_bool(
...
flags.DEFINE_integer(
    "top_down_camera_altitude", 40, "Altitude of the top-down world camera (in meters)."
)
...
flags.DEFINE_string(
    "simulation_recording_file", None, "Path to where the simulation is recorded"
)
flags.DEFINE_string("data_path", get_default_data_path(), "Path where to logged data")
flags.DEFINE_bool(
    "log_detector_output", False, "Enable recording of bbox annotated detector images"
)
flags.DEFINE_bool(
    "log_traffic_light_detector_output",
    False,
    "Enable recording of bbox annotated tl detector images",
)
flags.DEFINE_bool(
    "log_lane_detection_camera", False, "True to enable lane detection camera logging"
)
flags.DEFINE_integer(
    "log_every_nth_message", 1, "Control how often the script logs messages"
)
########################################
# Evaluation operators.
########################################
flags.DEFINE_bool("evaluation", False, "Enable end-to-end evaluation of the pipeline.")
flags.DEFINE_bool(
...
flags.DEFINE_bool(
    "evaluate_obstacle_tracking", False, "True to enable object tracking evaluation"
)
flags.DEFINE_bool("evaluate_prediction", False, "True to enable prediction evaluation")
flags.DEFINE_bool("evaluate_control", False, "True to enable control evaluation")
flags.DEFINE_bool("evaluate_fusion", False, "True to enable fusion evaluation")
flags.DEFINE_bool(
    "evaluate_segmentation", False, "True to enable segmentation evaluation"
)
...
def must_visualize():
    ...
    return (
        ...
        or FLAGS.visualize_depth_camera
        or FLAGS.visualize_lidar
        or FLAGS.visualize_imu
        or FLAGS.visualize_pose
        ...
        or FLAGS.visualize_segmentation
...
def must_add_depth_camera_sensor():
    ...
    """
    return (
        FLAGS.perfect_obstacle_detection
        or FLAGS.evaluate_obstacle_detection
        or FLAGS.perfect_depth_estimation
        or FLAGS.visualize_depth_camera
...
def must_add_segmented_camera_sensor():
    ...
    """
    return (
        FLAGS.evaluate_segmentation
        or FLAGS.perfect_segmentation
...
flags.register_multi_flags_validator(
    ...
    lambda flags_dict: (
        not flags_dict["obstacle_detection"]
        or (
            flags_dict["obstacle_detection"]
            and (
...
flags.register_multi_flags_validator(
    ...
    message="prediction requires --obstacle_tracking or --perfect_obstacle_tracking",
)

flags.register_multi_flags_validator(
    ["visualize_prediction", "execution_mode"],
    lambda flags_dict: (
        (not flags_dict["visualize_prediction"])
        or flags_dict["execution_mode"] == "simulation"
    ),
...
flags.register_multi_flags_validator(
    ...
    message="--prediction must be enabled when --evaluate_prediction is enabled",
)

flags.register_multi_flags_validator(
    ["evaluate_prediction", "prediction_num_future_steps", "tracking_num_steps"],
    lambda flags_dict: (
        not flags_dict["evaluate_prediction"]
        or (
            flags_dict["evaluate_prediction"]
            and flags_dict["prediction_num_future_steps"]
            <= flags_dict["tracking_num_steps"]
        )
    ),
...
flags.register_multi_flags_validator(
    ...
    message="waypoint planner requires obstacle detection and traffic light detection",
)
...
def obstacle_detection_validator(flags_dict):
    ...
    if flags_dict["simulator_obstacle_detection"]:
        return not (
            flags_dict["obstacle_detection"] or flags_dict["perfect_obstacle_detection"]
        )
...
def obstacle_tracking_evaluation_validator(flags_dict):
    if flags_dict["evaluate_obstacle_tracking"]:
        return flags_dict["obstacle_tracking"] and (
            flags_dict["perfect_obstacle_detection"] or flags_dict["obstacle_detection"]
        )
    return True
...
flags.register_multi_flags_validator(
    ...
    message="--segmentation must be set when --evaluate_segmentation is enabled",
)
...
flags.register_multi_flags_validator(
    ...
    depth_validator,
    message="--depth_estimation and --perfect_depth_estimation cannot be both set",
)
