...
def add_efficientdet_obstacle_detection(
    camera_stream, time_to_decision_stream, csv_file_name=None
):
...
def add_obstacle_detection(camera_stream, time_to_decision_stream, csv_file_name=None):
...
def add_yolo_obstacle_detection(
    camera_stream, time_to_decision_stream, csv_file_name=None
):
    """Adds an operator that uses a YOLOv8 model for obstacle detection."""
    from pylot.perception.detection.yolo_detection_operator import YoloDetectionOperator

    if csv_file_name is None:
        csv_file_name = FLAGS.csv_log_file_name
    op_config = erdos.OperatorConfig(
        name="yolo_detection_operator",
        flow_watermarks=False,
        log_file_name=FLAGS.log_file_name,
        csv_log_file_name=csv_file_name,
        profile_file_name=FLAGS.profile_file_name,
    ...
    obstacles_streams = erdos.connect(
        YoloDetectionOperator,
        op_config,
        [camera_stream, time_to_decision_stream],
        FLAGS.yolo_detection_model_path,
        FLAGS,
    )
    return obstacles_streams
...
def add_obstacle_location_finder(
    obstacles_stream, depth_stream, pose_stream, camera_setup
):
...
def add_detection_decay(ground_obstacles_stream):
    ...
    op_config = erdos.OperatorConfig(
        name="detection_decay_operator",
        log_file_name=FLAGS.log_file_name,
        csv_log_file_name=FLAGS.csv_log_file_name,
        profile_file_name=FLAGS.profile_file_name,
    )
    [map_stream] = erdos.connect(
        DetectionDecayOperator, op_config, [ground_obstacles_stream], FLAGS
    )
    return map_stream
...
def add_control_evaluation(pose_stream, waypoints_stream, name="control_eval_operator"):
    ...
    erdos.connect(
        ControlEvalOperator, op_config, [pose_stream, waypoints_stream], FLAGS
    )


def add_traffic_light_detector(traffic_light_camera_stream, time_to_decision_stream):
    ...
    [traffic_lights_stream] = erdos.connect(
        TrafficLightDetOperator,
        op_config,
...
def add_traffic_light_invasion_sensor(ground_vehicle_id_stream, pose_stream):
    ...
    [traffic_light_invasion_stream] = erdos.connect(
        CarlaTrafficLightInvasionSensorOperator,
        op_config,
        [ground_vehicle_id_stream, pose_stream],
        FLAGS,
    )
    return traffic_light_invasion_stream


def add_canny_edge_lane_detection(bgr_camera_stream, name="canny_edge_lane_detection"):
    ...
    [lane_detection_stream] = erdos.connect(
        CannyEdgeLaneDetectionOperator, op_config, [bgr_camera_stream], FLAGS
    )
...
def add_lanenet_detection(bgr_camera_stream, name="lanenet_lane_detection"):
    ...
    [lane_detection_stream] = erdos.connect(
        LanenetDetectionOperator, op_config, [bgr_camera_stream], FLAGS
    )
    return lane_detection_stream


def add_obstacle_tracking(
    obstacles_stream, bgr_camera_stream, time_to_decision_stream, name_prefix="tracker_"
):
    from pylot.perception.tracking.object_tracker_operator import ObjectTrackerOperator
...
def add_center_track_tracking(bgr_camera_stream, camera_setup, name="center_track"):
    from pylot.perception.tracking.center_track_operator import CenterTrackOperator
...
def add_qd_track_tracking(bgr_camera_stream, camera_setup, name="qd_track"):
    ...
    op_config = erdos.OperatorConfig(
        name="qd_track_operator",
        log_file_name=FLAGS.log_file_name,
        csv_log_file_name=FLAGS.csv_log_file_name,
        profile_file_name=FLAGS.profile_file_name,
    )
    [obstacle_tracking_stream] = erdos.connect(
        QdTrackOperator, op_config, [bgr_camera_stream], FLAGS, camera_setup
    )
    return obstacle_tracking_stream
...
def add_tracking_evaluation(
    ...
    from pylot.perception.tracking.tracking_eval_operator import TrackingEvalOperator
...
def add_depth_estimation(
    ...
    try:
        from pylot.perception.depth_estimation.depth_estimation_operator import (
    ...
    [depth_estimation_stream] = erdos.connect(
        DepthEstimationOperator,
        op_config,
...
def add_segmentation(bgr_camera_stream, name="drn_segmentation_operator"):
    ...
    [segmented_stream] = erdos.connect(
        SegmentationDRNOperator, op_config, [bgr_camera_stream], FLAGS
    )
    return segmented_stream


def add_segmentation_evaluation(
    ground_segmented_stream, segmented_stream, name="segmentation_evaluation_operator"
):
...
def add_segmentation_decay(ground_segmented_stream, name="segmentation_decay_operator"):
    ...
    [iou_stream] = erdos.connect(
        SegmentationDecayOperator, op_config, [ground_segmented_stream], FLAGS
    )
    return iou_stream
...
def add_linear_prediction(tracking_stream, time_to_decision_stream):
    ...
    [prediction_stream] = erdos.connect(
        ...
        op_config,
        [tracking_stream, time_to_decision_stream],
        FLAGS,
    )
    return prediction_stream


def add_r2p2_prediction(
    point_cloud_stream, obstacles_tracking_stream, time_to_decision_stream, lidar_setup
):
...
def add_prediction_evaluation(
    pose_stream, tracking_stream, prediction_stream, name="prediction_eval_operator"
):
...
def add_behavior_planning(
    ...
    from pylot.planning.behavior_planning_operator import BehaviorPlanningOperator
...
def add_left_right_cameras(transform, vehicle_id_stream, release_sensor_stream, fov=90):
...
def add_collision_sensor(vehicle_id_stream):
    ...
    [collision_stream] = erdos.connect(
        CarlaCollisionSensorDriverOperator, op_config, [vehicle_id_stream], FLAGS
    )
...
def add_lane_invasion_sensor(vehicle_id_stream):
    ...
    [lane_invasion_stream] = erdos.connect(
        CarlaLaneInvasionSensorDriverOperator, op_config, [vehicle_id_stream], FLAGS
    )
    return lane_invasion_stream


def add_camera_driver(camera_setup, vehicle_id_stream, release_sensor_stream):
    from pylot.drivers.carla_camera_driver_operator import CarlaCameraDriverOperator

    op_config = erdos.OperatorConfig(
        name=camera_setup.get_name() + "_operator",
        flow_watermarks=False,
        log_file_name=FLAGS.log_file_name,
        csv_log_file_name=FLAGS.csv_log_file_name,
        profile_file_name=FLAGS.profile_file_name,
...
def _add_lidar_driver(vehicle_id_stream, release_sensor_stream, lidar_setup):
    from pylot.drivers.carla_lidar_driver_operator import CarlaLidarDriverOperator
...
def add_imu(transform, vehicle_id_stream, name="imu"):
    ...
    [imu_stream] = erdos.connect(
        CarlaIMUDriverOperator, op_config, [vehicle_id_stream], imu_setup, FLAGS
    )
...
def add_gnss(transform, vehicle_id_stream, name="gnss"):
    ...
    [gnss_stream] = erdos.connect(
        CarlaGNSSDriverOperator, op_config, [vehicle_id_stream], gnss_setup, FLAGS
    )
    return (gnss_stream, gnss_setup)


def add_localization(imu_stream, gnss_stream, ground_pose_stream, name="localization"):
...
def add_fusion(pose_stream, obstacles_stream, depth_stream, ground_obstacles_stream):
    ...
    [obstacle_pos_stream] = erdos.connect(
        FusionOperator, op_config, [pose_stream, obstacles_stream, depth_stream], FLAGS
    )
...
def add_mpc(pose_stream, waypoints_stream):
    ...
    [control_stream] = erdos.connect(
        MPCOperator, op_config, [pose_stream, waypoints_stream], FLAGS
    )
...
def add_pid_control(pose_stream, waypoints_stream):
    ...
    [control_stream] = erdos.connect(
        PIDControlOperator, op_config, [pose_stream, waypoints_stream], FLAGS
    )
    return control_stream
...
def add_planning_pose_synchronizer(
    waypoint_stream, pose_stream, localization_stream, notify_stream1, notify_stream2
):
...
def add_bounding_box_logging(
    obstacles_stream, file_base_name, name="bounding_box_logger_operator"
):
    ...
    [finished_indicator_stream] = erdos.connect(
        BoundingBoxLoggerOperator, op_config, [obstacles_stream], FLAGS, file_base_name
    )
    return finished_indicator_stream
...
def add_camera_logging(stream, name, filename_prefix):
    ...
    [finished_indicator_stream] = erdos.connect(
        CameraLoggerOperator, op_config, [stream], FLAGS, filename_prefix
    )
    return finished_indicator_stream
...
def add_chauffeur_logging(
    ...
    op_config = erdos.OperatorConfig(
        name="chauffeur_logger_operator",
        log_file_name=FLAGS.log_file_name,
        csv_log_file_name=FLAGS.csv_log_file_name,
        profile_file_name=FLAGS.profile_file_name,
    )
...
def add_eval_metric_logging(
    ...
    """
    from pylot.loggers.eval_metric_logger_operator import EvalMetricLoggerOperator

    op_config = erdos.OperatorConfig(
        name="eval_metric_logger_operator",
        log_file_name=FLAGS.log_file_name,
        csv_log_file_name=FLAGS.csv_log_file_name,
        profile_file_name=FLAGS.profile_file_name,
    )
...
def add_lidar_logging(
    point_cloud_stream, name="lidar_logger_operator", filename_prefix="lidar"
):
    ...
    [finished_indicator_stream] = erdos.connect(
        LidarLoggerOperator, op_config, [point_cloud_stream], FLAGS, filename_prefix
    )
    return finished_indicator_stream
...
def add_multiple_object_tracker_logging(
    ...
    [finished_indicator_stream] = erdos.connect(
        MultipleObjectTrackerLoggerOperator, op_config, [obstacles_stream], FLAGS
    )
    return finished_indicator_stream


def add_trajectory_logging(
    obstacles_tracking_stream, name="trajectory_logger_operator"
):
...
def add_visualizer(
    ...
    if camera_stream is None or not (
        FLAGS.visualize_rgb_camera
    ...
    if traffic_lights_stream is None or not (
        FLAGS.visualize_detected_traffic_lights or FLAGS.visualize_world
    ):
        traffic_lights_stream = erdos.IngestStream()
        streams_to_send_top_on.append(traffic_lights_stream)
    if tracked_obstacles_stream is None or not FLAGS.visualize_tracked_obstacles:
    ...
    if prediction_stream is None or not (
        FLAGS.visualize_prediction or FLAGS.visualize_world
    ):
        prediction_stream = erdos.IngestStream()
        streams_to_send_top_on.append(prediction_stream)
    if waypoints_stream is None or not (
        FLAGS.visualize_waypoints or FLAGS.visualize_world
    ):
...
def add_perfect_detector(
    ...
    op_config = erdos.OperatorConfig(
        name="perfect_detector_operator",
        log_file_name=FLAGS.log_file_name,
        csv_log_file_name=FLAGS.csv_log_file_name,
        profile_file_name=FLAGS.profile_file_name,
    )
...
def add_perfect_traffic_light_detector(
...
def add_perfect_lane_detector(pose_stream, open_drive_stream, center_camera_stream):
    ...
    op_config = erdos.OperatorConfig(
        name="perfect_lane_detection_operator",
        log_file_name=FLAGS.log_file_name,
        csv_log_file_name=FLAGS.csv_log_file_name,
        profile_file_name=FLAGS.profile_file_name,
    )
    [detected_lanes_stream] = erdos.connect(
        PerfectLaneDetectionOperator,
        op_config,
...
def add_perfect_tracking(vehicle_id_stream, ground_obstacles_stream, pose_stream):
    from pylot.simulation.perfect_tracker_operator import PerfectTrackerOperator

    op_config = erdos.OperatorConfig(
        name="perfect_tracking_operator",
        log_file_name=FLAGS.log_file_name,
        csv_log_file_name=FLAGS.csv_log_file_name,
        profile_file_name=FLAGS.profile_file_name,
    )
...
def add_time_to_decision(pose_stream, obstacles_stream):
    ...
    [time_to_decision] = erdos.connect(
        TimeToDecisionOperator, op_config, [pose_stream, obstacles_stream], FLAGS
    )
