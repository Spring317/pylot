...
def add_obstacle_detection(
    ...
    if FLAGS.obstacle_detection:
        ...
        if FLAGS.obstacle_detector_model == "yolo":
            logger.debug("Using YOLO obstacle detector...")
            obstacles_streams = pylot.operator_creator.add_yolo_obstacle_detection(
                center_camera_stream, time_to_decision_stream
            )
            obstacles_stream_wo_depth = obstacles_streams[0]
        elif FLAGS.obstacle_detector_model == "efficientdet":
...
def add_traffic_light_detection(
    ...
    if FLAGS.traffic_light_detection or FLAGS.perfect_traffic_light_detection:
        ...
        (tl_camera_stream, _) = pylot.operator_creator.add_camera_driver(
    ...
    if FLAGS.traffic_light_detection:
        ...
        traffic_lights_stream = pylot.operator_creator.add_obstacle_location_finder(
            traffic_lights_stream, depth_stream, pose_stream, tl_camera_setup
        )

    if FLAGS.perfect_traffic_light_detection:
        assert pose_stream is not None and ground_traffic_lights_stream is not None
        ...
        tl_depth_camera_setup = DepthCameraSetup(
            "traffic_light_depth_camera",
            FLAGS.camera_image_width,
            FLAGS.camera_image_height,
            tl_transform,
            45,
        )
        (tl_depth_camera_stream, _) = pylot.operator_creator.add_camera_driver(
            tl_depth_camera_setup, vehicle_id_stream, release_sensor_stream
        )
        ...
        (tl_segmented_camera_stream, _) = pylot.operator_creator.add_camera_driver(
            segmented_tl_camera_setup, vehicle_id_stream, release_sensor_stream
        )
...
def add_lane_detection(center_camera_stream, pose_stream=None, open_drive_stream=None):
    ...
    if FLAGS.lane_detection:
        if FLAGS.lane_detection_type == "canny":
            ...
            lane_detection_stream = pylot.operator_creator.add_lanenet_detection(
                center_camera_stream
            )
        else:
            raise ValueError(
                "Unexpected lane detection type {}".format(FLAGS.lane_detection_type)
            )
        ...
        if FLAGS.tracker_type == "center_track":
            ...
            obstacles_wo_history_tracking_stream = (
                pylot.operator_creator.add_obstacle_tracking(
                    obstacles_stream, center_camera_stream, time_to_decision_stream
                )
            )
...
def add_segmentation(center_camera_stream, ground_segmented_stream=None):
    ...
    if FLAGS.segmentation:
        ...
        segmented_stream = pylot.operator_creator.add_segmentation(center_camera_stream)
...
def add_prediction(
    ...
    if FLAGS.prediction:
        if FLAGS.prediction_type == "linear":
            ...
            raise ValueError(
                "Unexpected prediction_type {}".format(FLAGS.prediction_type)
            )
        ...
        if FLAGS.visualize_prediction:
            ...
            (top_down_segmented_camera_stream, notify_reading_stream) = (
                pylot.operator_creator.add_camera_driver(
                    top_down_seg_camera_setup, vehicle_id_stream, release_sensor_stream
                )
            )
    else:
        logger.debug("Not using prediction...")
    return (prediction_stream, top_down_segmented_camera_stream, notify_reading_stream)
...
def add_planning(
    ...
    trajectory_stream = pylot.operator_creator.add_behavior_planning(
        pose_stream, open_drive_stream, global_trajectory_stream, goal_location
    )
...
def add_control(
    ...
    if FLAGS.control == "pid":
        ...
        control_stream = pylot.operator_creator.add_mpc(pose_stream, waypoints_stream)
        ...
        if FLAGS.evaluate_obstacle_detection and not FLAGS.perfect_obstacle_detection:
            # Ensure that the perfect obstacle detector doesn't remain
            # behind.
            logger.debug("Synchronizing ticking using the perfect detector stream")
    ...
    if FLAGS.evaluate_control:
        ...
        pylot.operator_creator.add_control_evaluation(pose_stream, waypoints_stream)
