...
flags.DEFINE_integer(
    "obstacle_detection_gpu_index", 0, "The index of the GPU to deploy the model on"
)
flags.DEFINE_float(
...
flags.DEFINE_string(
    "path_coco_labels", "dependencies/models/pylot.names", "Path to the COCO labels"
)
flags.DEFINE_float(
...
# YOLO detection flags.
flags.DEFINE_string(
    "yolo_detection_model_path",
...
flags.DEFINE_float(
    "traffic_light_det_min_score_threshold", 0.3, "Min score threshold for bounding box"
)
flags.DEFINE_float(
...
flags.DEFINE_integer(
    "traffic_light_det_gpu_index", 0, "The index of the GPU to deploy the model on"
)
...
flags.DEFINE_float("eval_min_matching_iou", 0.5, "IoU required for tracking evaluation")
flags.DEFINE_integer(
    "obstacle_track_max_age", 3, "Number of frames to track without a detection update"
)
flags.DEFINE_integer(
...
flags.DEFINE_integer(
    "lane_detection_gpu_index", 0, "The index of the GPU to deploy the model on"
)
flags.DEFINE_string(
...
flags.DEFINE_integer(
    "detection_eval_lookback", None, "Number of images from the past to aggregate"
)
flags.DEFINE_integer(
    "detection_eval_freq", None, "Give rolling detection eval every this many images"
)
