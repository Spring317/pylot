...
###############################################################################
# Conda env creation/activation is skipped (the 'pylot' env already exists and
# is assumed to be ACTIVATED). These vars/guards lived only in the commented-out
# block above, but the steps below still reference them.
CONDA_ENV_NAME="pylot"

# $PYLOT_HOME drives every absolute path below; fall back to the cwd.
if [ -z "$PYLOT_HOME" ]; then
    PYLOT_HOME=$(pwd)
    echo "WARNING: \$PYLOT_HOME is not set; Setting it to ${PYLOT_HOME}"
fi

# The planner cmake builds and the CARLA egg hook need an active conda env.
if [ -z "$CONDA_PREFIX" ]; then
    echo "ERROR: No conda environment is active."
    echo "       Run 'conda activate ${CONDA_ENV_NAME}' before this script."
    exit 1
fi
